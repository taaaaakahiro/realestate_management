...
- **収支 登録/編集/削除** — 年・月・収入/支出・**科目**でフィルター、ページネーション、**CSV／PDF エクスポート**、**複数件の一括登録**。ローン返済は適用金利で内訳自動分解。収支・損益は色分け表示
- **回収予測** — 直近の家賃収入ペースで、取得原価をあと何年で回収できるかを試算
- **繰り上げ返済** — 期間短縮型／返済額軽減型に対応（残債・利息・完済予定に反映）
